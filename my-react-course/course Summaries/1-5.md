...
>7.) To use with scss - should: </br>
  >> a.) Run npm install node-sass </br>
  >> b.) Change test: /\.css$/,   to   test: /\.scss$/,</br>
  >> c.) Rename the folder from css to scss and  file to *.scss </br>
  >> d.) In main.js import '../scss/style.scss';
