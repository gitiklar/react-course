...
7.) To use with scss - should: </br> >> a.) run npm install node-sass </br> >> b.) change test: /\.css$/, to  test: /\.scss$/,</br>>> c.) rename the folder from css to scss and file to *.scss </br>>> d.) import '../scss/style.scss';
